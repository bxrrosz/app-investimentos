...
if usd_brl is not None:
    for tk in df_precos.columns:
        if not tk.endswith(".SA"):                    # internacional
            serie = df_precos[tk].dropna()
            if serie.empty:
                continue
            serie.index = pd.to_datetime(serie.index)
            taxa_alinh = usd_brl.reindex(serie.index, method="ffill")
            df_precos.loc[serie.index, tk] = serie * taxa_alinh
    st.info("Conversão p/ BRL concluída (USD‑BRL).")
else:
    st.info("Sem taxa USD‑BRL: ativos internacionais ficam na moeda original.")

# opcional debug ↓
# st.write(df_precos.head())
# st.write(usd_brl.head() if usd_brl is not None else "USD‑BRL nulo")
...
with col2:
    ...
    for tk in tickers:
        ...
        price = df_precos[tk].dropna().iloc[-1] if tk in df_precos else np.nan
        pos = qtd * price
        inv = qtd * pm
        val_tot += pos
        inv_tot += inv
